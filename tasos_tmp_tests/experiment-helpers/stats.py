...
def gather_perf_mode():
    ...
    for fname in glob.glob(FUZZER_STAT_FILES_GLOB):
        with open(fname) as f:
            ...
            run_time = str(run_time) if "day" in str(run_time) else "0 day, " + str(run_time)
            results.append(
    ...
    if results:
        ...
        print(f"Avg total time: {str(timedelta(seconds=avg_total_seconds)).split('.')[0]}")
        print(f"Iterations per second: {1000000/avg_total_seconds:.2f}")
