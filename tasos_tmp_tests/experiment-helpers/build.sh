...
gettext-base libelf-dev libc6-dbg
...
make -j
cp ./sabre "${FINISH_DIR}/sabre"
cp ./plugins/sbr-afl/libsbr-afl.so "${FINISH_DIR}/libsbr-afl-non.so"

cmake -DCMAKE_BUILD_TYPE=RELEASE -DSF_MEMFS=ON -DSF_STDIO=OFF -DSF_SLEEP=OFF -DSF_SMARTDEFER=OFF ..
make -j
cp ./sabre "${FINISH_DIR}/sabre"
cp ./plugins/sbr-afl/libsbr-afl.so "${FINISH_DIR}/libsbr-afl-only-fs.so"

cmake -DCMAKE_BUILD_TYPE=RELEASE -DSF_MEMFS=OFF -DSF_STDIO=ON -DSF_SLEEP=OFF -DSF_SMARTDEFER=OFF ..
make -j
cp ./sabre "${FINISH_DIR}/sabre"
cp ./plugins/sbr-afl/libsbr-afl.so "${FINISH_DIR}/libsbr-afl-only-stdio.so"

cmake -DCMAKE_BUILD_TYPE=RELEASE -DSF_MEMFS=OFF -DSF_STDIO=OFF -DSF_SLEEP=ON -DSF_SMARTDEFER=OFF ..
make -j
cp ./sabre "${FINISH_DIR}/sabre"
cp ./plugins/sbr-afl/libsbr-afl.so "${FINISH_DIR}/libsbr-afl-only-sleep.so"

cmake -DCMAKE_BUILD_TYPE=RELEASE -DSF_MEMFS=OFF -DSF_STDIO=OFF -DSF_SLEEP=OFF -DSF_SMARTDEFER=ON ..
make -j
cp ./sabre "${FINISH_DIR}/sabre"
cp ./plugins/sbr-afl/libsbr-afl.so "${FINISH_DIR}/libsbr-afl-only-defer.so"
